...
codelist.xml A simple enumerated code list directly equivalent to those
in version 2.1 and earlier.

> This example shows the SDMX:CL_AGE ‘Age’ cross domain code list.

codelist – extended.xml Extended code list.
...
codelist – discriminated union.xml Discriminated union of code lists.
...
valuelist.xml A simple enumerated value list.
...
conceptscheme.xml The example illustrates a single concept scheme
ECB:ECB_CONCEPTS containing multiple individual concepts.

**Data Structure Definition**

ECB EXR.xml The example illustrates a data structure definition (DSD)
...
dataflow.xml A simple data flow for the ECB:EXR data set.

**Geospatial**

geospatial geocomponents.xml An example of the use of the
...
geospatial geographiccodelist.xml An example of the definition of
...
geospatial geogridcodelist.xml An example of the definition of
...
VTL Sample 1.xml Illustrates the use of VTL Rulesets to validate an SDMX
...
VTL Sample 2.xml Illustrates aggregation of an SDMX dataflow using VTL
...
VTL Sample 3.xml Illustrates the calculation of a GDP per capita dataset
...
ECB EXR.xml Structure specific data message.  
...
ECB EXR Dataflow.xsd Structure specific schema for validating the
dataset.
...
EXB EXR CA.xml Structure specific data message.  
...
> SOURCE_AGENCY – coded series-level array attribute specifying a
...
ECB EXR CA Dataflow.xsd Structure specific schema for validating the
dataset.
...
ECB EXR CA DSD.xml Structure message describing the Data Structure
